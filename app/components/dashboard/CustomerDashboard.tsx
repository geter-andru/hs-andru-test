...
interface AsyncState {
  [key: string]: AsyncOperation
}

interface AsyncAction {
  type: 'START_OPERATION' | 'COMPLETE_OPERATION' | 'ERROR_OPERATION' | 'RESET_OPERATION'
  operation: string
  result?: any
  error?: string
}
...
const CustomerDashboard: React.FC = () => {
  ...
  const [dashboardLayout, setDashboardLayout] = useState<'progressive' | 'integrated'>('progressive')
  const [showMobileSidebar, setShowMobileSidebar] = useState(false)
  const [isMobile, setIsMobile] = useState(false)

  // State reducer to handle race conditions in async operations
  const [asyncState, dispatchAsync] = useReducer((state: AsyncState, action: AsyncAction): AsyncState => {
    switch (action.type) {
      ...
      case 'ERROR_OPERATION':
        return { ...state, [action.operation]: { loading: false, error: action.error || 'Unknown error' } }
  ...
  const toolCallbacks: ToolCallbacks = {
    onICPComplete: useCallback(async (data: ICPCompleteData) => {
      try {
        ...
        dispatchAsync({ type: 'ERROR_OPERATION', operation: 'icpComplete', error: error instanceof Error ? error.message : 'Unknown error' })
        throw error
      }
    ...
    onCostCalculated: useCallback(async (data: CostCalculatedData) => {
      try {
        ...
        dispatchAsync({ type: 'ERROR_OPERATION', operation: 'costCalculated', error: error instanceof Error ? error.message : 'Unknown error' })
        throw error
      }
    }, [completeTool, handleToolCompletion, checkUnlockCriteria, showProgressRecognition, showMilestoneReached, showToolAccessEarned, handleTabChange]),

    onBusinessCaseReady: useCallback(async (data: BusinessCaseReadyData) => {
      try {
        ...
        dispatchAsync({ type: 'ERROR_OPERATION', operation: 'businessCaseReady', error: error instanceof Error ? error.message : 'Unknown error' })
        throw error
      }
    ...
    <DashboardAccessControl customerData={workflowData}>
      {shouldUseProgressiveEngagement ? (
        ...
        <ProgressiveEngagementContainer
          ...
          onToolCompletion={async (toolName: string, data: any) => {
