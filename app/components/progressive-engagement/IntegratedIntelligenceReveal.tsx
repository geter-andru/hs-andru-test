...
interface CustomerData {
  icp_score?: number;
  icp_tier?: string;
  estimated_cost_delay?: number;
  estimated_roi?: number;
}

interface CompetencyData {
  // Add competency data structure as needed
  [key: string]: any;
}
...
interface RevelationState {
  framework: IntegratedFramework | null;
  isRevealing: boolean;
  showDataFlow: boolean;
  showSophistication: boolean;
...
interface DataFlow {
  icpAnalysis: DataFlowItem;
  costCalculation: DataFlowItem;
  businessCase: DataFlowItem;
}
...
interface SophisticationMetrics {
  dataPointsIntegrated: number;
  crossReferences: number;
  automatedInsights: number;
  stakeholderViews: number;
}

interface Integration {
  connections: Connection[];
  sophisticationMetrics: SophisticationMetrics;
}

interface ConsultingEquivalent {
  timeUsuallyRequired: string;
...
interface IntegratedFramework {
  ...
  consultingEquivalent: ConsultingEquivalent;
  nextLevelCapabilities: NextLevelCapability[];
}

interface IntegratedIntelligenceRevealProps {
  customerId: string;
  completedAnalysisData: CompletedAnalysisData;
  onAdvancedAccess: (capabilityName: string, accessData?: any) => void;
}

const IntegratedIntelligenceReveal: React.FC<IntegratedIntelligenceRevealProps> = ({ 
  customerId, 
  completedAnalysisData, 
  onAdvancedAccess 
}) => {
  const [revelationState, setRevelationState] = useState<RevelationState>({
    framework: null,
    isRevealing: false,
    showDataFlow: false,
  ...
  const [animationSequence, setAnimationSequence] = useState<number>(0);

  useEffect(() => {
    const initializeIntegratedFramework = async () => {
      try {
        // Simulate data loading with fallback approach for Next.js
        const framework = buildIntegratedFramework(
          {}, // Mock customer data
  ...
  const buildIntegratedFramework = (
  ...
  const buildFallbackFramework = (): IntegratedFramework => ({
    achievement: {
      title: "Complete Strategic Framework Established",
      description: "Comprehensive revenue intelligence system constructed",
      completionTime: "15 minutes",
  ...
  const triggerRevelationSequence = () => {
    // Professional confetti celebration (browser only)
    if (typeof window !== 'undefined') {
      // Dynamic import for confetti to handle client-side only
      import('canvas-confetti').then((confetti) => {
        ...
        console.log('Confetti celebration attempted');
      });
    }
  ...
  const handleAdvancedAccess = (capability: NextLevelCapability) => {
