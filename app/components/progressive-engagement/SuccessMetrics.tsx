...
interface CompellingAspectEngagement {
  [aspectType: string]: number;
}

interface EngagementMetrics {
  timeToFirstInteraction: number | null;
  compellingAspectEngagement: CompellingAspectEngagement;
...
interface CompletionMetrics {
  ...
  integrationReached: boolean;
}

interface SatisfactionMetrics {
  interactionDepth: number;
  returnEngagement: boolean;
  advancedFeatureAccess: number;
}
...
interface UseSuccessMetricsReturn {
  ...
  getProgressionBottlenecks: () => ProgressionBottleneck[];
...
export const useSuccessMetrics = (customerId: string): UseSuccessMetricsReturn => {
  ...
  const trackEngagement = (eventType: string, data: any = {}) => {
  ...
  // Send metrics to backend (mock implementation for Next.js)
  const persistMetrics = async () => {
    try {
      ...
      // Mock persistence - in real implementation, would call API
      console.log('Persisting success metrics:', metricsData);
    } catch (error) {
  ...
  const getEngagementInsights = (): EngagementInsight[] => {
    const insights: EngagementInsight[] = [];
    
    // Time to engagement analysis
    if (metrics.engagement.timeToFirstInteraction !== null) {
  ...
  const getProgressionBottlenecks = (): ProgressionBottleneck[] => {
    const bottlenecks: ProgressionBottleneck[] = [];
        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
          <div className="metric-card text-center">
            <div className="text-2xl font-bold text-blue-400">
              {metrics.engagement.timeToFirstInteraction !== null ? 
