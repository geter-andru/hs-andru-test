...
interface WelcomeHeroProps {
  ...
  onStartEngagement?: (engagement: string) => void;
}
...
const WelcomeHero: React.FC<WelcomeHeroProps> = ({ 
  ...
  const [icpAnalysisData, setIcpAnalysisData] = useState<ICPAnalysisData | null>(null);
  ...
  useEffect(() => {
    const loadPersonalizedWelcome = async () => {
      try {
        // Mock data loading for Next.js compatibility
  ...
  const calculateEstimatedValue = (data: CustomerData): number => {
  ...
  const generateCompellingPreviews = (data: CustomerData): Opportunity[] => {
  ...
  const handleHighlightClick = (toolType: string) => {
    try {
      if (onStartEngagement) {
      ...
      >
        ...
        >
          <button
            ...
            className="w-full sm:w-auto bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white font-bold text-lg py-4 px-8 rounded-xl transition-all duration-300 disabled:opacity-50"
          >
            {isNavigating ? 'Loading...' : 'See Full Analysis & Rating Tools'}
          </button>
        </motion.div>
