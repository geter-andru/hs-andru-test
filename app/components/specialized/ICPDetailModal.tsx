...
const ICPDetailModal: React.FC<ICPDetailModalProps> = ({ 
  isOpen, 
  onClose, 
  icpContent, 
  onProgressUpdate,
  ...
  const [ratedCompanies, setRatedCompanies] = useState<Map<string, number>>(new Map());
  ...
  const awardPoints = useCallback((points: number, action: string, category: string = 'customerAnalysis') => {
  ...
  const handleCompanyRating = useCallback((companyId: string, rating: number) => {
          ...
          <div className="bg-gray-900 border-b border-gray-800 p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h1 id="icp-modal-title" className="text-xl font-semibold text-white">
...
const CompanyRatingCard: React.FC<CompanyRatingCardProps> = ({ company, currentRating, onRate }) => {
        ...
        <div className="flex space-x-1">
          {ratingOptions.map((rating) => (
            <button
              ...
              className={`px-3 py-1 rounded text-sm font-medium transition-colors min-h-[36px] touch-manipulation ${
                currentRating === rating
                  ? 'bg-blue-600 text-white'
                  : 'bg-gray-600 text-gray-300 hover:bg-gray-500'
              }`}
              aria-label={`Rate ${rating} out of 100`}
            >
              {rating}
            </button>
          ))}
...
const PersonaDetailView: React.FC = () => (
  <div className="bg-gray-800 rounded-lg p-6">
...
export type { ICPDetailModalProps, Company, ProgressUpdate, SectionConfig, CompanyRatingCardProps };