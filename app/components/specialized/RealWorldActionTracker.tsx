...
const RealWorldActionTracker: React.FC<RealWorldActionTrackerProps> = ({ 
  ...
  // Load completed actions (would come from API in production)
  useEffect(() => {
    // Mock data for demonstration
    const mockActions: CompletedAction[] = [
      {
        id: 'action_1',
  ...
  const handleSubmitAction = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!actionType || !impactLevel) return;
      ...
      // In production, would track with API service
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-lg p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            ...
            className="flex items-center space-x-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-blue-600/50 disabled:cursor-not-allowed text-white rounded-lg font-medium transition-colors min-h-[44px] touch-manipulation"
            aria-label="Log new action"
          >
        ...
        {categoryFilters.map(filter => {
          ...
          return (
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2 rounded-lg font-medium transition-colors min-h-[36px] touch-manipulation ${
                isActive 
      ...
      <div className="space-y-4">
        {filteredActions.length === 0 ? (
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-8 text-center">
            ...
            <button
              onClick={() => setShowAddAction(true)}
              className="px-6 py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-blue-600/50 disabled:cursor-not-allowed text-white rounded-lg font-medium transition-colors min-h-[44px] touch-manipulation"
              aria-label="Log your first action"
            >
          ...
          filteredActions.map((action) => {
            ...
            const colors = getColorClasses(actionType?.color || 'gray');
                  ...
                  <div className="flex items-start space-x-4 flex-1">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center space-x-4 text-sm text-gray-500">
                        ...
                        <span className={`px-2 py-1 rounded text-xs ${getColorClasses(impactLevel?.color || 'gray').bgSecondary} ${getColorClasses(impactLevel?.color || 'gray').text}`}>
      ...
      <AnimatePresence>
        {showAddAction && (
          ...
          >
            ...
            >
              <div className="p-6">
                <h3 id="action-modal-title" className="text-xl font-semibold text-white mb-6">Log Real-World Action</h3>
                ...
                <form onSubmit={handleSubmitAction} className="space-y-6">
                      ...
                      {actionTypes.map((type) => {
                        ...
                        return (
                          <button
                            ...
                            className={`flex items-start space-x-3 p-3 rounded-lg border text-left transition-colors min-h-[44px] touch-manipulation ${
                              isSelected 
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                      required
                    />
                  </div>
                      ...
                      {impactLevels.map((level) => {
                        ...
                        return (
                          <button
                            ...
                            className={`p-3 rounded-lg border text-left transition-colors min-h-[44px] touch-manipulation ${
                              isSelected 
                                ? `${colors.border} ${colors.bgSecondary}` 
                                : 'border-gray-700 hover:border-gray-600'
                            }`}
                            aria-label={`Select ${level.name} impact`}
                          >
                            <div className={`font-medium ${isSelected ? colors.text : 'text-white'}`}>
                              {level.name}
                            </div>
                  ...
                  <div className="flex items-center justify-end space-x-3">
                    <button
                      type="button"
                      onClick={() => setShowAddAction(false)}
                      className="px-4 py-2 text-gray-400 hover:text-white transition-colors min-h-[44px] touch-manipulation"
                      aria-label="Cancel action logging"
                    >
                      Cancel
                    </button>
                    ...
                    <button
                      ...
                      className="px-6 py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-gray-700 disabled:text-gray-500 disabled:cursor-not-allowed text-white rounded-lg font-medium transition-colors min-h-[44px] touch-manipulation"
                      aria-label="Submit action log"
                    >
                      Log Action
                    </button>
                  </div>
