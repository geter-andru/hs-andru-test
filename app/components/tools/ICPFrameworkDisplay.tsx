...
interface ICPFrameworkDisplayProps {
  customerData?: any; // Can be more specific based on your customer data structure
  onFrameworkUpdate?: (framework: FrameworkCriterion[]) => void;
}

const ICPFrameworkDisplay: React.FC<ICPFrameworkDisplayProps> = ({ 
  customerData, 
  onFrameworkUpdate 
}) => {
  ...
  const handleWeightChange = (id: number, newWeight: string): void => {
  ...
  const handleToggleCriterion = (id: number): void => {
    setCriteria(prev => prev.map(c => 
  ...
  const handleAddCriterion = (): void => {
    if (newCriterionName.trim()) {
      const newCriterion: ICPCriterion = {
        id: Date.now(),
  ...
  const handleDeleteCriterion = (id: number): void => {
                    ...
                    <div className="flex items-center space-x-2">
                      {isCustomizing && (
                        <button
                          ...
                          aria-label={criterion.enabled ? 'Disable criterion' : 'Enable criterion'}
                        >
