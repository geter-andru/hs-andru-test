...
import type { FounderProfile, FounderScalingStatus, SystematicScalingSession } from '@/app/lib/services/customerValueOrchestrator';
...
export const SystematicScalingProvider: React.FC<SystematicScalingProviderProps> = ({
  ...
  useEffect(() => {
    const initializeFounderData = async () => {
      ...
      try {
        ...
        if (initialProfile) {
          const profile = await customerValueOrchestrator.initializeFounderProfile(founderId, initialProfile);
  ...
  const initializeFounder = useCallback(async (
    ...
    setIsLoading(true);
    try {
      const profile = await customerValueOrchestrator.initializeFounderProfile(founderId, profileData);
      setFounderProfile(profile);
      await refreshScalingStatus();
    } catch (error) {
  ...
  const processToolUsage = useCallback(async (
    ...
    try {
      const result = await customerValueOrchestrator.processToolUsage(founderId, toolUsed, sessionData);
  ...
  const executeSystematicAction = useCallback(async (
    ...
    setIsProcessingAction(true);
    try {
      const result = await customerValueOrchestrator.executeSystematicAction(
        founderId,
        actionType,
        actionDetails
      );
  ...
  const getSystematicRecommendations = useCallback(async () => {
    ...
    return await customerValueOrchestrator.getSystematicRecommendations(founderId);
  }, [founderId]);
  ...
  const generateScalingPlan = useCallback(async (
    ...
    setIsProcessingAction(true);
    try {
      const plan = await customerValueOrchestrator.generateScalingPlan(founderId, horizon);
  ...
  const refreshScalingStatus = useCallback(async () => {
    ...
    try {
      const [status, assessment, intelligence] = await Promise.all([
        customerValueOrchestrator.getFounderScalingStatus(founderId),
  ...
  const getCompetencyProgress = useCallback((area: UserCompetency['area']) => {
    ...
    const competency = scalingStatus.competencyProfile.find(c => c.area === area);
  ...
  const getCompetencyLevel = useCallback((competency: string) => {
    const competencyData = scalingStatus?.competencyProfile.find(c => c.area === competency);
    return {
