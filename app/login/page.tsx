...
import NativeSupabaseAuth from '../components/auth/NativeSupabaseAuth';

type AuthMode = 'customer' | 'marketing' | 'supabase';
...
function LoginContent() {
  ...
  const handleCustomerSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (customerId) {
      // Enhanced authentication with token support from React SPA
      if (accessToken) {
        // Use enhanced auth system with token
        const result = await import('@/lib/api/client').then(module => 
          module.auth.login(customerId, accessToken)
        );
        
        if (result.success) {
          // Enhanced admin detection for routing
        ...
        // Traditional customer ID login through enhanced API
                ...
                <form onSubmit={handleCustomerSubmit} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Enter your Customer ID (e.g., dru78DR9789SDF862)"
                      className="w-full px-lg py-md border border-glass-border rounded-md bg-surface text-text-primary placeholder-text-subtle focus:outline-none focus:ring-2 focus:ring-brand-primary transition-normal"
                      required
                    />
                  </div>
                  ...
                  <div className="bg-glass-background border border-glass-border rounded-md p-3">
                    <div className="flex items-start space-x-2">
                      ...
                      <div className="text-xs text-text-muted">
                        ...
                        <p><code className="bg-surface px-1 py-0.5 rounded text-text-primary">dru78DR9789SDF862</code> (Admin Demo)</p>
                        <p><code className="bg-surface px-1 py-0.5 rounded text-text-primary">dru45XY1234ABC567</code> (Basic Access)</p>
                      </div>
                    </div>
                  </div>
                </div>
